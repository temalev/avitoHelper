<template>
  <header>
    <div class="title pointer" @click="$router.push({ name: 'home' })">{{ $route.meta.title }}</div>
    ...
    <Dialog v-model:visible="showPanel" modal header="Edit Profile" :style="{ width: '25rem' }">
      ...
      <div class="p-text-secondary block mb-5">
        Введите сумму и способ пополнения <br />
      ...
      <div class="d-flex-column align-items-center gap-3 mb-2">
        ...
        <div v-for="type in paymentType" :key="type.id" class="flex align-center">
          <RadioButton v-model="form.type_id" :inputId="type.id" name="dynamic" :value="type.id" />
